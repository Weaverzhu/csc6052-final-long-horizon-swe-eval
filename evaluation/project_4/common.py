...
DEFAULT_DATA_PATH = "knowledge_base_data.json"
DEFAULT_INDEX_PATH = "knowledge_base_index.json"
...
class AgentProject:
    ...
    def index_path(self) -> Path:
        return self.root / DEFAULT_INDEX_PATH
    ...
    def assert_submission_contract(self) -> None:
        ...
        assert package_dir is not None, (
            "submission must define a package at knowledge_base/ or src/knowledge_base/"
        )
        ...
        assert implementation_modules, (
            "submission must keep CLI wiring thin and place knowledge-base logic in separate modules"
        )
    ...
    def run_cli(self, *args: str, timeout: int = 10) -> subprocess.CompletedProcess[str]:
        ...
        try:
            return subprocess.run(
                ["uv", "run", "python", "-m", "knowledge_base", *args],
                capture_output=True,
                cwd=self.root,
                env=env,
                text=True,
...
def note_record(*, id: str, title: str, body: str, tags: list[str]) -> dict[str, Any]:
    return {
        "id": id,
        "title": title,
        "body": body,
...
def seed_basic_notes(project: AgentProject) -> None:
    assert_success(
        project.run_cli(
            "note",
            "add",
            ...
            "Search Design",
            "--body",
            "Indexed retrieval for benchmark notes.",
        )
    )
    assert_success(
        project.run_cli(
            ...
            "Alpha Spec",
            "--body",
            "Course project baseline and glossary.",
        )
    )
    assert_success(project.run_cli("note", "tag", "--id", "N-001", "--tag", "core"))
    assert_success(project.run_cli("note", "tag", "--id", "N-001", "--tag", "research"))
