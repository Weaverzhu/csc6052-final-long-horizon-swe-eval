...
class AgentProject:
    ...
    def assert_submission_contract(self) -> None:
        ...
        assert package_dir is not None, (
            "submission must define a package at course_planner/ or src/course_planner/"
        )
        ...
        assert implementation_modules, (
            "submission must keep CLI wiring thin and place planner logic in separate modules"
        )
    ...
    def run_cli(self, *args: str, timeout: int = 10) -> subprocess.CompletedProcess[str]:
        ...
        try:
            return subprocess.run(
                ["uv", "run", "python", "-m", "course_planner", *args],
                capture_output=True,
                cwd=self.root,
                env=env,
                text=True,
...
def seed_basic_catalog(project: AgentProject, *, data_path: str | None = None) -> None:
    suffix = ["--data", data_path] if data_path is not None else []
    assert_success(
        project.run_cli(
            "course",
            "add",
